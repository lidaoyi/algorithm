...
func main() {
	...
	node := obj.deserialize(str)

	fmt.Println("---------------")
	fmt.Println(obj.serialize(node))
}
...
func (this *Codec) serialize(root *TreeNode) string {
	...
	maxIdx := 0
	idxMap := make(map[int]*TreeNode)
	maxIdx = putToArr(root, 0, idxMap, maxIdx)
	...
	for i := 0; i <= maxIdx; i++ {
		item, ok := idxMap[i]
		if ok {
			buf.WriteString(strconv.FormatInt(int64(item.Val), 10))
		} else {
...
func putToArr(node *TreeNode, idx int, arr map[int]*TreeNode, idxMax int) int {
	fmt.Printf("idx: %d Left: %T, Right: %T \n", idx, node.Left, node.Right)
	arr[idx] = node
	idxMax = idx
	if node.Left != nil {
		max := putToArr(node.Left, idx*2+1, arr, idxMax)
		if max > idxMax {
			idxMax = max
		}
	}
	if node.Right != nil {
		max := putToArr(node.Right, idx*2+2, arr, idxMax)
		if max > idxMax {
			idxMax = max
		}
	}
...
func (this *Codec) deserialize(data string) *TreeNode {
	...
	elements := strings.Split(data1, ",")
	fmt.Printf("data1 : %s elements: %s \n", data1, elements)

	size := len(elements)
	fmt.Printf("size : %d elements: %s \n", size, elements)

	root := TreeNode{}
	root.Val = parseString(elements[0])
	arr := make([]*TreeNode, size)
	arr[0] = &root
	for i := 1; i < size; i++ {
		...
		node := TreeNode{}
		node.Val = parseString(elements[i])

		arr[i] = &node
		father := (i - 1) / 2
		left := (i-1)%2 == 0
		if left {
			arr[father].Left = &node
		} else {
			arr[father].Right = &node
		}
